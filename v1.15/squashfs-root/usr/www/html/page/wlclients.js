...
var G_EnableAllowedMACAddresses="<?get :InternetGatewayDevice.LANDevice.1.WLANConfiguration.1.MACAddressControlEnabled?>";
var G_W_Status = [];
var n = 0;
...
function createTable(){
	...
	for(var i = 0, _len = G_W_Status.length; i < _len; i++){	
		for(var j=0; j<MACAddressesList.length; j++){		
			if(G_EnableAllowedMACAddresses == "1" && MACAddressesList[j].length == 17 && MACAddressesList[j] != G_W_Status[i][1])
				continue;
			else if(G_W_Status[i][2] == '1'){   //  strlen("00:00:00:00:00:00")==17;
