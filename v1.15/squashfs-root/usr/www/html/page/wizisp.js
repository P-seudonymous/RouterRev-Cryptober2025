...
function autoSearchPVC(){
	...
	if(dslstatus != 'Up')
	{
		alert(data_language.dynamic.DSLstatus);
	    return false;
	}
	$('Scan').disabled = true;
...
function ajax_getPvc(_text){
	...
	if (G_Error == '0' && G_FoundPvc != ''){
		...
		$('lang_pvcresult').innerHTML = 'Found Available PVC ' + G_FoundPvc + '.';
		//assignment
		...
		$('display_access').style.display = 'none';
		$('display_result').style.display = 'block';
		$('lang_pvcresult').innerHTML = data_language.dynamic.no_pvc;
		$('Scan').disabled = false;
		return false;
	}	
}

function ClearStatue(){
	$H({
		':InternetGatewayDevice.X_TWSZ-COM_Diagnostics.AutoPVCSearch.Enable': '0',
		'obj-action' : 'set',
		'var:page'   : 'wizisp',
		'getpage': 'html/page/autopvc_ajax.js',
		'errorpage'  : 'html/page/autopvc_ajax.js'
	...
	var _url = "/cgi-bin/webproc";
	ajax = Ajax.getInstance(_url, "", 0, ajax_Clear);
	ajax.post($('uiPostForm'));
}
...
function uiNextPage(){
	...
	{ // -> DSL
		...
		{
			alert(data_language.dynamic.vpi);
			return false;
		}
		if(($('INPUT_VCI').value < 32) ||($('INPUT_VCI').value > 65535))
		{
			alert(data_language.dynamic.vci);
			return false;
		}
	}
