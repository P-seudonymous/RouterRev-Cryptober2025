...
function uiSubmit(){
		...
		/*检测不能添加一模一样的URL条目 lbw*/
		for(var i=0;i<G_BlackListNum;i++)
	    {
			//新条目的URL在列表中已经存在
			if(G_URLBlackList[i][1] == $('INPUT_URLAddress').value)
			{
				//如果新条目的调度为select模式
				if(Form.Radio('RADIO_ScheduleMode')=='Select')
				{
					//如果扫描的条目的调度方式也是select,并且选择的调度方式一样，则说明条目重复，错误返回。
					if(G_URLBlackList[i][2] == 'Select' && $('SELECT_Schedules').value == G_URLBlackList[i][3])
					{
                         		alert("Rules already exist.");
					     return false;
					 }
				}//如果新条目为menual模式，则需要比较时间。
				else if(starttime ==G_URLBlackList[i][7] && endtime==G_URLBlackList[i][8] && selectdays == G_URLBlackList[i][5] && G_URLBlackList[i][3].indexOf("Never")==-1)
                {
					    alert("Rules already exist.");
					    return false;
				}
