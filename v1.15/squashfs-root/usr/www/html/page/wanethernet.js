...
var Wanethernet_Enable = '<?get :InternetGatewayDevice.WANDevice.3.WANEthernetInterfaceConfig.Enable?>' ;

//Wan Conns----$var:secIdx ->第二层；$var:thdIdx -> 第三层
var G_WanConns = [];
//var m = 0;
<?if eq $var:conn_type `Bridge`
`	<?mget :InternetGatewayDevice.WANDevice.3.WANConnectionDevice.$var:secIdx.WANIPConnection.$var:thdIdx. "Enable Name NATEnabled ExternalIPAddress SubnetMask DefaultGateway DNSOverrideAllowed X_TWSZ-COM_UsrDNSServers AddressingType ConnectionType X_TELEFONICA-ES_NATType DNSServers X_TWSZ-COM_ServiceList"
	`	G_WanConns['Path'] = "InternetGatewayDevice.WANDevice.3.WANConnectionDevice.$var:secIdx.WANIPConnection.$var:thdIdx."; //Path
		...
		G_WanConns['X_TWSZ-COM_ServiceList']   = "$0d"; //X_TWSZ-COM_ServiceList
	`?>
`?>
...
`	<?mget :InternetGatewayDevice.WANDevice.3.WANConnectionDevice.$var:secIdx.WANPPPConnection.$var:thdIdx. "Enable Name NATEnabled Username PPPAuthenticationProtocol ConnectionTrigger IdleDisconnectTime MaxMRUSize PPPLCPEchoRetry X_TWSZ-COM_StaticIPAddress X_TELEFONICA-ES_NATType PPPLCPEcho MaxMTUSize X_TWSZ-COM_ServiceList"
	`	G_WanConns['Path'] = "InternetGatewayDevice.WANDevice.3.WANConnectionDevice.$var:secIdx.WANPPPConnection.$var:thdIdx.";//Path
		G_WanConns['Enable']                    = "$01";//Enable
		G_WanConns['Name']                      = "$02";//Name
		G_WanConns['NATEnabled']                = "$03";//NATEnabled
		G_WanConns['Username']                  = "$04";//Username
	    G_WanConns['Password']                  = "**********";//Password
	    G_WanConns['PPPAuthenticationProtocol'] = "$05";//PPPAuthenticationProtocol
	    G_WanConns['ConnectionTrigger']         = "$06";//ConnectionTrigger
	    G_WanConns['IdleDisconnectTime']        = "$07";//IdleDisconnectTime
	    G_WanConns['MaxMRUSize']                = "$08";//MaxMRUSize
	    G_WanConns['PPPLCPEchoRetry']           = "$09";//PPPLCPEchoRetry
	    G_WanConns['X_TWSZ-COM_StaticIPAddress']= "$0a";//X_TWSZ-COM_StaticIPAddress
	    G_WanConns['X_TELEFONICA-ES_NATType']        = "$0b";//X_TELEFONICA-ES_NATType
	    G_WanConns['PPPLCPEcho']                = "$0c";//PPPLCPEcho
		G_WanConns['MaxMTUSize']                = "$0d";//MaxMTUSize
		G_WanConns['X_TWSZ-COM_ServiceList']   = "$0e"; //X_TWSZ-COM_ServiceList
	`?>
`?>
<?if eq $var:G_IPv6Enable 1
`   <?if eq $var:conn_type `PPPOE`
...
function onClickWanSeviceType(protocol){
	...
	if(protocol.indexOf('PPP') > -1)
	{
	    ...
	    if(_split){
		 $('INPUT_ServiceName').value = _split[0] + '_' + _split[1]+ '_' + _split[2] + '_' +value_wanServiceType1;
	    }
	    ...
	    var _split = value_name.split('_');
	    if(_split){
		 $('INPUT_ServiceName').value = _split[0] + '_' + _split[1]+ '_' + _split[2] + '_' +value_wanServiceType2;
	    }
...
`?>
	...
	if(G_WanConns['Name'])
	{
		var _split =G_WanConns['Name'].split('_');
	    if(_split){
			$('INPUT_ServiceName').value = '_' + $('INPUT_VlanId').value+ '_' + _split[2];
	    }
	}
	switch(_protocol){
		case 'PPPoE' : {
			...
			<?if eq 1 $var:mod_WAN_SERVICETYPE
			`
			$('WanServiceType1').style.display = 'block';
			$('WanServiceType2').style.display = 'none';
			`?>
			...
			setJSONValue({
				...
				'INPUT_MRUSize'       : G_WanConns['MaxMRUSize'] || '1492',
				'INPUT_MTUSize'       : G_WanConns['MaxMTUSize'] || '1492',
				...
				<?if neq 1 $var:mod_WAN_SERVICETYPE
				`
					'INPUT_ServiceName'   : G_WanConns['Name'] ? ('pppoe' + $('INPUT_ServiceName').value) : ('pppoe_' + ($('INPUT_VlanId').value || '0') + '_' + num)
				`?>
				<?if eq 1 $var:mod_WAN_SERVICETYPE
				`
					'INPUT_ServiceName'   : G_WanConns['Name'] ? ('pppoe' + $('INPUT_ServiceName').value + '_' + $('SELECT_WANServiceType_1').value) : ('pppoe_' + ($('INPUT_VlanId').value || '0') + '_' + num + '_' + ($('SELECT_WANServiceType_1').value||'Internet_TR069'))
				`?>
			});
		...
		case 'DHCP' : {
			...
			<?if eq 1 $var:mod_WAN_SERVICETYPE
			`
			$('WanServiceType2').style.display = 'block';
			`?>
			if(wan_service_type == 1){
			...
			setJSONValue({
				...
				<?if neq 1 $var:mod_WAN_SERVICETYPE
				`		
				'INPUT_ServiceName'     : G_WanConns['Name'] ? ('mer' + $('INPUT_ServiceName').value) : ('mer_' + ($('INPUT_VlanId').value || '0') + '_' + num )
				`?>
				<?if eq 1 $var:mod_WAN_SERVICETYPE
				`		
				'INPUT_ServiceName'     : G_WanConns['Name'] ? ('mer' + $('INPUT_ServiceName').value + '_' + $('SELECT_WANServiceType_2').value) : ('mer_' + ($('INPUT_VlanId').value || '0') + '_' + num + '_' + ($('SELECT_WANServiceType_2').value||'Internet_TR069'))
				`?>
			});
			if(G_WanConns['ConnectionType'] == 'IP_Bridged'){
				Form.Checkbox('INPUT_EnableNat', '1');
...
`?>
		...
		case 'Bridging' : {
			...
			`?>
			if(G_WanConns['Name'])
				$('INPUT_ServiceName').value = ('br' + $('INPUT_ServiceName').value);
			else
				$('INPUT_ServiceName').value = 'br_' + ($('INPUT_VlanId').value || '0') + '_' + num;
...
function onchgSetVlan(){
	...
	if(_split){
		if(wan_service_type == 1){
			$('INPUT_ServiceName').value = _split[0] + '_' + value_vlanid + '_' + _split[2] +  '_' + ($('SELECT_Protocol').value == 'PPPoE' ? $('SELECT_WANServiceType_1').value : $('SELECT_WANServiceType_2').value ||'internet');
		}
		else{
...
function uiAjax_create(_text){
	...
	if (Wanethernet_Error == '0'){
		uiAjax_create_start();		
	} else if(Wanethernet_Error == '1'){
		dealWithError();
...
function uiAjax_create_start(){
	...
	switch(_protocol){
		case 'PPPoE' : {
			_node = $('INPUT_UserName','INPUT_Psd','SELECT_AuthMethod','INPUT_AcName','INPUT_PPPServiceName','INPUT_InaTimeout','INPUT_MRUSize',
					  'INPUT_PPP_IP_Ext','INPUT_IPAddress','INPUT_IpAddr_PPP','SELECT_WanIntf_PPP','INPUT_LCPEchoInterval','INPUT_LCPEchoRetry','INPUT_MTUSize');
...
function ajax_AddProc(_text){
	...
	if (Wanethernet_Error == '0' && G_Error != '1'){
		$G('/cgi-bin/webproc',{
	...
	} else if (Wanethernet_Error == '1' || G_Error == '1'){
