...
var G_FilterMode = "<?get :InternetGatewayDevice.X_TWSZ-COM_MAC_Filter_Wlan.WlanFilterMode?>";
...
`	G_Wireless[SSID_num] = [":InternetGatewayDevice.LANDevice.1.WLANConfiguration.$00", //Path
					 "$01" //SSID
					 ];
	SSID_num++;
`?>
...
<?objget :InternetGatewayDevice.X_TWSZ-COM_MAC_Filter_Wlan.WlanBlackList. "WlanMACAddress WlanScheduleMode WlanSchedulePath WlanEnableAllDays WlanDays WlanEnableAllTimes WlanStartTime WlanEndTime WlanDescription WlanSSIDIndex"
`	G_MacBlackList[G_BlackListNum] = [];
	G_MacBlackList[G_BlackListNum][0] = "InternetGatewayDevice.X_TWSZ-COM_MAC_Filter_Wlan.WlanBlackList.$00.";  				//index
	G_MacBlackList[G_BlackListNum][1] = "$01";		//MAC
	G_MacBlackList[G_BlackListNum][2] = "$02";		//WlanScheduleMode	
	G_MacBlackList[G_BlackListNum][3] = "$03";		//WlanSchedulePath
	G_MacBlackList[G_BlackListNum][4] = "$04";		//WlanEnableAllDays
	G_MacBlackList[G_BlackListNum][5] = "$05";		//WlanDays
	G_MacBlackList[G_BlackListNum][6] = "$06";		//WlanEnableAllTimes
	G_MacBlackList[G_BlackListNum][7] = "$07";		//WlanStartTime
	G_MacBlackList[G_BlackListNum][8] = "$08";		//WlanEndTime
	G_MacBlackList[G_BlackListNum][9] = "$09";		//UserName
	G_MacBlackList[G_BlackListNum][10] = "$0a";		//WlanSSIDIndex
	G_BlackListNum++;
`?>

<?objget :InternetGatewayDevice.X_TWSZ-COM_MAC_Filter_Wlan.WlanWhiteList. "WlanMACAddress WlanScheduleMode WlanSchedulePath WlanEnableAllDays WlanDays WlanEnableAllTimes WlanStartTime WlanEndTime WlanDescription WlanSSIDIndex"
`	G_MacWhiteList[G_WhiteListNum] = [];
	G_MacWhiteList[G_WhiteListNum][0] = "InternetGatewayDevice.X_TWSZ-COM_MAC_Filter_Wlan.WlanWhiteList.$00.";  				//index
	G_MacWhiteList[G_WhiteListNum][1] = "$01";		//MAC
	G_MacWhiteList[G_WhiteListNum][2] = "$02";		//WlanScheduleMode	
	G_MacWhiteList[G_WhiteListNum][3] = "$03";		//WlanSchedulePath
	G_MacWhiteList[G_WhiteListNum][4] = "$04";		//WlanEnableAllDays
	G_MacWhiteList[G_WhiteListNum][5] = "$05";		//WlanDays
	G_MacWhiteList[G_WhiteListNum][6] = "$06";		//WlanEnableAllTimes
	G_MacWhiteList[G_WhiteListNum][7] = "$07";		//WlanStartTime
	G_MacWhiteList[G_WhiteListNum][8] = "$08";		//WlanEndTime
	G_MacWhiteList[G_WhiteListNum][9] = "$09";		//UserName
	G_MacWhiteList[G_WhiteListNum][10] = "$0a";		//WlanSSIDIndex
	G_WhiteListNum++;
`?>
...
function uiRadioSubmit()
{
	...
	var path=':InternetGatewayDevice.X_TWSZ-COM_MAC_Filter_Wlan.WlanFilterMode';		
	$F(path,G_FilterMode);
...
function createTable(){	
	...
	for(var i = 0; i < G_MacBlackList.length; i++){
		...
		var interface = getSSID(G_MacBlackList[i][10]);
		array_MACAddrs[i].push(interface);	
	...
	for(var i = 0; i < G_MacWhiteList.length; i++){
		...
		var interface = getSSID(G_MacWhiteList[i][10]);
		array_PassMACAddrs[i].push(interface);	
	}
...
function createSSIDOpt(){
	...
	for(var i = 1, _len = G_Wireless.length; i <_len; i++){
...
function uiSubmit(){	
    ...
    {
		$H({
			'add-obj' 	 	: G_FilterMode=='Deny'? 'InternetGatewayDevice.X_TWSZ-COM_MAC_Filter_Wlan.WlanBlackList.' : 'InternetGatewayDevice.X_TWSZ-COM_MAC_Filter_Wlan.WlanWhiteList.',
			':WlanDescription'       	: $('INPUT_UserName').value,
			':WlanMACAddress'       : MACAddress,
			':WlanScheduleMode' 	: Form.Radio('RADIO_ScheduleMode')=='Select'?'Select':'Manual',
			':WlanSchedulePath'		: Form.Radio('RADIO_ScheduleMode')=='Select'? $('SELECT_Schedules').value : undefined,
			':WlanEnableAllDays'   	: ScheduleMode=='Manual' ? Form.Radio('EnableAllDays')=='1'?1:0 :undefined ,
			':WlanDays'      		: ScheduleMode=='Manual' ? selectdays : undefined,
			':WlanEnableAllTimes'   : ScheduleMode=='Manual' ? Form.Checkbox('EnableAllTimes'): undefined,
			':WlanStartTime' 		: ScheduleMode=='Manual' ? starttime : undefined,
			':WlanEndTime'   		: ScheduleMode=='Manual' ? endtime : undefined,
			':WlanSSIDIndex'   		: $('SELECT_SSID').value,
			'obj-action' 		: 'add-set',
			'var:menu'			: G_Menu,
			'var:page'			: G_Page,
			'var:subpage'		: G_SubPage,
	...
	{		
		...
		var path=':' + pathTemp;
		$F(path+'WlanDescription',$('INPUT_UserName').value);
		$F(path+'WlanMACAddress',MACAddress);
		if(Form.Radio('RADIO_ScheduleMode')=='Select')
        {
			$F(path+'WlanScheduleMode','Select');
			$F(path+'WlanSchedulePath',$('SELECT_Schedules').value);
		}
        else
        {
			$F(path+'WlanScheduleMode','Manual');
			$F(path+'WlanEnableAllDays',Form.Radio('EnableAllDays')=='1'?1:0);
			$F(path+'WlanDays',selectdays);
			$F(path+'WlanEnableAllTimes',Form.Checkbox('EnableAllTimes'));
			$F(path+'WlanStartTime',starttime);
			$F(path+'WlanEndTime',endtime);
		}
		$F(path+'WlanSSIDIndex',$('SELECT_SSID').value);
	}
...
function EditEntry(){
	...
	for(var j = 0; j < G_MacBlackList.length; j++){
		...
		if(EditIndex=='1'){
			...
			$('SELECT_SSID').value = G_MacBlackList[j][10];
...
function EditEntryWhitelist()
{
	...
	for(var j = 0; j < G_MacWhiteList.length; j++){
		...
		if(EditIndex=='1'){
			...
			$('SELECT_SSID').value = G_MacWhiteList[j][10];
...
function RemoveEntryWhilte()
{
	...
	for(var j = 0; j < G_MacWhiteList.length; j++){
		...
		var DelIndex=Form.Checkbox(DelIndexID);
		if(DelIndex=='1'){
       		FilterMode = "<?get :InternetGatewayDevice.X_TWSZ-COM_MAC_Filter_Wlan.WlanFilterMode?>";
