...
var G_IGMPStatus = "<?get :InternetGatewayDevice.X_TELEFONICA-ES_IGMPProxy.Enabled?>";
...
function Clear_Internet_Time_Counter()
{
	...
	$H({
		'var:menu'				: G_Menu,
		'var:page'				: G_Page,
		'var:subpage'			: 'deviceinfo',
		'var:errorpage'			: 'deviceinfo',
		'obj-action'			: 'set',
...
function load_adsl_list()
{
	...
	for(var i = 0; i < G_wanConnction_adsl.length; i++){
		...
		`	
			array_value[i].push(((G_ATM[j][5] == '1') && (G_IGMPStatus == '1'))? 'Enable' : 'Disable');   //igmp
		`?>
	...
	for(var i = 0; i < G_wanConnction_adsl.length; i++){
		if ((G_DefaultRouter != "") && (G_wanConnction_adsl[i][0].indexOf(G_DefaultRouter) > -1))
		{
			j = getAtmParam_adsl(i);
			type_adsl = get_conn_type_adsl(i);

			array_value_gw[i] = [];
			array_value_gw[i].push(G_ATM[j][2]);         //vci/vpi
			array_value_gw[i].push(G_wanConnction_adsl[i][2]);  //name
			array_value_gw[i].push(type_adsl);   //type
			array_value_gw[i].push(G_wanConnction_adsl[i][8]);   //ip addr
		}
	}
	$T('Current_GW_info',array_value_gw);
}

function load_eth_list()
{
	var array_value_eth = [];
	var array_value_eth_GW = [];
	var igmp = '';
	var type_eth = '';
	...
	for(var k = 0; k < G_wanConnction_eth.length; k++){
		...
		<?if eq 1 $var:mod_TF_IGMP
		`
			array_value_eth[k].push(((G_ATM_ETH[j][1] == '1') && (G_IGMPStatus == '1')) ? 'Enable' : 'Disable');   //igmp
		`?>
	...
	for(var k = 0; k < G_wanConnction_eth.length; k++){
		if ((G_DefaultRouter != "") && (G_wanConnction_eth[k][0].indexOf(G_DefaultRouter) > -1))
		{
			j = getAtmParam_eth(k);
			type_eth = get_conn_type_eth(k);

			array_value_eth_GW[k] = [];
			array_value_eth_GW[k].push("N/A");         //vci/vpi
			array_value_eth_GW[k].push(G_wanConnction_eth[k][2]);  //name
			array_value_eth_GW[k].push(type_eth);   //type
			array_value_eth_GW[k].push(G_wanConnction_eth[k][8]);   //ip addr
		}
	
	}
	$T('Current_GW_info',array_value_eth_GW);
...
function load_3g_list()
{
	...
	for(var i = 0; i < G_wanConnction_3g.length; i++){
		...
		<?if eq 1 $var:mod_TF_IGMP
		`	
			array_value[i].push(((G_ATM_3G[j][5] == '1') && (G_IGMPStatus == '1')) ? 'Enable' : 'Disable');   //igmp
		`?>
	...
	for(var i = 0; i < G_wanConnction_3g.length; i++){
		if ((G_DefaultRouter != "") && (G_wanConnction_3g[i][0].indexOf(G_DefaultRouter) > -1))
		{
			j = getAtmParam_3g(i);
			type_3g = get_conn_type_3g(i);

			array_value_GW[i] = [];
			array_value_GW[i].push("N/A");         //vci/vpi
			array_value_GW[i].push(G_wanConnction_3g[i][2]);  //name
			array_value_GW[i].push(type_3g);   //type
			array_value_GW[i].push(G_wanConnction_3g[i][8]);   //ip addr
		}
	}
	$T('Current_GW_info',array_value_GW);
}
