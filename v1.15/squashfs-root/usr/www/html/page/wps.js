...
function checkBeaconType(){
	var beaconT = get_auth_mode();
	if(Form.Checkbox('INPUT_WpsEnable') && beaconT == 'WEP')
	{
		alert("The current security mode is WEP, the WPS will be invalid");
	}
}
...
function uiOK()
{
	...
	var _path1 = ':InternetGatewayDevice.LANDevice.1.WLANConfiguration.1';
 	<?if eq 1 $var:mod_WPS2X
	`
	...
	if($('SELECT_ConfigurationState').value == "Not configured" && G_WPS[0][10] == "Configured")
	{
		$F(_path1 + '.SSID','D-Link');
		$F(_path1 + '.Channel','0');
		$F(_path1 + '.BeaconType'             , 'None');
		$F(_path1 + '.BasicEncryptionModes'   , 'None');
		$F(_path1 + '.BasicAuthenticationMode', 'None');
	}
...
function uiCancel()
{
	$('display_config').style.display = "block";
	$('display_reset').style.display = "none";	
}
...
function uiSubmit()
{
	...
	if($('SELECT_ConfigurationState').value == "Not configured" && G_WPS[0][10] == "Configured")
	{	
		$('display_config').style.display = "none";
		$('display_reset').style.display = "block";
	}
	else
	{
		uiOK();	
