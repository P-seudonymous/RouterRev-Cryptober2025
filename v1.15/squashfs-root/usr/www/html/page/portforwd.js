        ...
        if($('SELECT_WanConns').value == G_RACL[i][7])
        {
        	//if(G_RACL[i][1] == '1')
        	{
	            ...
	            && protocol == G_RACL[i][4])
	            {
	                ...
	                {
	                	if(confirm("The port 21 is already use in wan side \"Access Control Service\", do you want to use the port 21 to create a FTP virtual server?")){
								
						}
						else
						{
							document.location.href = '/cgi-bin/webproc?getpage=html/index.html&var:menu=maintenance&var:page=accessctrl&var:subpage=services';
							return false;
						}
	                }
	                else
...
function appSelect(sName) {
   ...
   with (document.vsFrm) {
      ...
      var protocol = proto[j].value == 1 ? 'TCP' : 'UDP';
      for(var k = 0;  k< G_RACL.length; k++)
    {
        if($('SELECT_WanConns').value == G_RACL[k][7])
        {
        	if((sName == 'Web Server (HTTP)' || sName =='TFTP' || sName =='FTP Server'))
        	{
				if(eStart[j].value == 80 && G_RACL[k][6]==80 && protocol ==G_RACL[k][4]) //HTTP
				{ 
					 alert("The port is confilict with Access Control Service, HTTP server port will be moved to 8080");
                     $('eStartEdit').value= eStart[j].value = '8080';
    	    	      $('eEndEdit').value= eEnd[j].value = '8080';
    	    	      $('iStartEdit').value = iStart[j].value = '8080';
    	    	      $('iEndEdit').value = iEnd[j].value = '8080';
					  
					  break;
				}
				if(eStart[j].value == 69 && G_RACL[k][6]==69 && protocol ==G_RACL[k][4]) //TFTP
				{
                     alert("The port is confilict with Access Control Service, TFTP server port will be moved to 6969");
                     $('eStartEdit').value= eStart[j].value = '6969';
    	    	      $('eEndEdit').value= eEnd[j].value = '6969';
    	    	      $('iStartEdit').value = iStart[j].value = '6969';
    	    	      $('iEndEdit').value = iEnd[j].value = '6969';
					  break;
				}
