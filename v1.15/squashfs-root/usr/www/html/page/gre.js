...
<?objget :InternetGatewayDevice.X_TELEFONICA-ES_GreTunnel. "TunnelName AssociatedWanIfName RemoteIpv4WanAddress PeerLocalAddress PeerRemoteAddress PeerIPv4Address PeerIPv4MaskLen LanPort"
`
    G_IpTunnel[m] = [];
...
{	
	...
	for(var i = 0; i < G_IpTunnel.length; i++)
	{
		...
		array_value[m].push((G_IpTunnel[i][8]).ellipsis(10)); //LanPort
		m++;
	}
	...
	for(i = 0; i < G_IpTunnel.length; i++){
		$('Iptunnel_table_' + i + '7').title = G_IpTunnel[i][8];
	}
...
function AddTunnel(){
	...
	var usednode = [];
	var n=0;
	var node_lanports = ['Lan1','Lan2','Lan3','Lan4','Wlan1','Wlan2','Wlan3','Wlan4'];
	
	for(var i = 0; i < G_IpTunnel.length; i++)
	{
		if(G_IpTunnel[i][8] != '')
		{
		    usedport = G_IpTunnel[i][8].split(',');
			for(var j = 0; j < usedport.length; j++)
			{
			    usednode[n] = usedport[j];
				n++;
			}
		}
	}
...
function EditTunnel(){
	...
	if(G_IpTunnel[editTunnelIdx][8] != "")
	{
		ownport = G_IpTunnel[editTunnelIdx][8].split(',');

		for(var m = 0; m < ownport.length; m++)
		{
		    $(ownport[m]).disabled = false;
	        $(ownport[m]).checked  = true;
		}
	}
}
...
function uiTunnelSubmit(){
	...
	if(lanport != '')
	{
	    lanport = lanport.substr(0,lanport.length -1);
	}
