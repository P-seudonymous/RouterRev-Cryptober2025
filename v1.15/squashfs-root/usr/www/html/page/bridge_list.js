...
function uiOnload(){
	...
	if(AllowEnable == 'Deny')
	{
		$('lang_dsc3_warning').style.display = 'block';
	}
	else
	{
		$('lang_dsc3_warning').style.display = 'none';
	}
...
function MacModeApply() {
	...
	if(MacStr != AllowEnable){
		var truthBeTold = window.confirm("It will delete all old rules in the list, are you sure to change?");
		if (truthBeTold) { 
		} else{
			disCtrl("select_mode",true);
			disCtrl("apply_rule",true);
