...
`?>	
	//when only 11n mode, should disable wep configuring
	if(G_Standard =='n')
	{
		_text   = ['None','Auto(WPA or WPA2)','WPA2 only','WPA only'];
		_value  = ['None','WPAand11i','11i','WPA'];
	}
	else
	{
		_text   = ['None','WEP','Auto(WPA or WPA2)','WPA2 only','WPA only'];
		_value  = ['None','Basic','WPAand11i','11i','WPA'];
	}
	$S('SELECT_Mode', _text, _value);
...
function uiSubmit(){
	...
	//WPS1.x
	if((G_WPS[Idx][1] == 1) && (G_WPS[Idx][2]== 1))
	{

		if(_nodes[0].value == 'Basic')
		{
			if(confirm("If the security is WEP, the WPS would be invalid, Do you sure?"))
			{
